...
```{r tidyweather}
#Select a subset of the dataset and save it as df
...
scatter_plot <- ggplot(tidyweather, aes(x=date, y = delta))+
  geom_point()+
...
#We add a facet wrap to distinguish between the various months
...
#plotting the data:
Yearly_scatter <- ggplot(average_annual_anomaly, aes(x = Year, y = mean_delta))+
  geom_point() +
...
summarise(mean_delta = mean(delta),
          ...
          higher = mean_delta + t_critical*SE_delta,
          )

#print out formula_CI
formula_ci

#create a conficence interval using bootstrapping method
library(infer)
set.seed(1234)
...
mean of the temperature delta is aroung 1.06 degrees. By means of a
...
and a degrees of freedom of 1 to calculate the critical values required
for a confidence interval calculation. We confirmed this estimate by
using a bootstrapping method that manually created the confidence
intervals and provided a lower bound of 1.02 degrees and a upper bound
of 1.11 degrees.
...
#select only the necessary data and mutate the data frame to include only the most important information
select_approval_polllist <- approval_polllist %>% 
  select(enddate, approve, disapprove, subgroup) %>% 
  mutate(delta = approve - disapprove)
...
ggplot(select_approval_polllist, aes(x = enddate, y = delta)) +
  facet_wrap(~ subgroup) +
  geom_line()
